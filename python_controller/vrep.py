...
class VRepLibDetectError(Exception):
  pass

# Load library
# Automatic library detection is a modification of the base V-REP Python API.
simx_path = ""
if platform.system() =='Windows':
  lib_path = "C:/Program Files (x86)/V-REP3/V-REP_PRO_EDU/programming/remoteApiBindings/lib/lib/"
  if os.path.exists("./remoteApi.dll"):
    simx_path = "./remoteApi.dll"
    print("Detected V-REP API DLL in cwd")
  elif os.path.exists(lib_path):
    if sizeof(c_voidp) == 4:  # 32-bit
      simx_path = os.path.join(lib_path, "32Bit", "remoteApi.dll")
      print("Detected V-REP install, 32-bit Python")
    elif sizeof(c_voidp) == 8:  # 64-bit
      simx_path = os.path.join(lib_path, "64Bit", "remoteApi.dll")
      print("Detected V-REP install, 64-bit Python")
    else:
      raise VRepLibDetectError("Unable to determine 32 or 64-bit Python")
  else:
    print("Cannot find V-REP API DLL")
elif platform.system() == 'Darwin':
  # Sorry Mac users, you'll have to do this the hard way!
  simx_path = "./remoteApi.dylib"
else:
  home = os.path.expanduser("~")
  lib_path = "programming/remoteApiBindings/lib/lib"
  if os.path.exists("./remoteApi.so"):
    simx_path = "./remoteApi.so"
    print("Detected V-REP API DLL in cwd")
  else:
    if os.path.exists(os.path.join(home, "V-REP_PRO_EDU_V3_2_0_rev6_64_Linux")):
      lib_path = os.path.join(home, "V-REP_PRO_EDU_V3_2_0_rev6_64_Linux", lib_path)
    elif os.path.exists(os.path.join(home, "V-REP_PRO_EDU_V3_2_0_rev6_32_Linux")):
      lib_path = os.path.join(home, "V-REP_PRO_EDU_V3_2_0_rev6_32_Linux", lib_path)
    else:
      raise VRepLibDetectError("Unable to detect V-REP install directory")
    
    if sizeof(c_voidp) == 4:  # 32-bit
      simx_path = os.path.join(lib_path, "32Bit", "remoteApi.so")
      print("Detected V-REP install, 32-bit Python")
    elif sizeof(c_voidp) == 8:  # 64-bit
      simx_path = os.path.join(lib_path, "64Bit", "remoteApi.so")
      print("Detected V-REP install, 64-bit Python")
    else:
      raise VRepLibDetectError("Unable to determine 32 or 64-bit Python")

libsimx = CDLL(simx_path)
